...
void poll_imu_sensor_fusion_quaternion_only() {
	for (std::variant<LSM6DSV16X::NoData, RotationQuaternion, GyroBiasVector, GravityVector> sensorFusionMeasurement;
	     !std::holds_alternative<LSM6DSV16X::NoData>(sensorFusionMeasurement = imu.get_measurement_sensor_fusion());) {
		if (!std::holds_alternative<RotationQuaternion>(sensorFusionMeasurement)) {
			continue;
		}

		auto const rotationQuaternion = std::get<RotationQuaternion>(sensorFusionMeasurement);
		Serial.print("Q: ");
		Serial.print(rotationQuaternion.rx, 6);
		Serial.print(',');
		Serial.print(rotationQuaternion.ry, 6);
		Serial.print(',');
		Serial.print(rotationQuaternion.rz, 6);
		Serial.print(',');
		Serial.println(rotationQuaternion.rw, 6);
	}
}

void poll_imu_fifo_all_supported_data() {
	int consecutiveNoDataCount = 0;
	while (consecutiveNoDataCount < 16) {
		bool hasData = false;

		auto const fifoMeasurement = imu.get_measurement_fifo();
		if (std::holds_alternative<AccelerationDataRaw>(fifoMeasurement)) {
			auto const accelerationData = std::get<AccelerationDataRaw>(fifoMeasurement);
			Serial.print("A: ");
			Serial.print(accelerationData.ax);
			Serial.print(',');
			Serial.print(accelerationData.ay);
			Serial.print(',');
			Serial.println(accelerationData.az);
			hasData = true;
		} else if (std::holds_alternative<GyroDataRaw>(fifoMeasurement)) {
			auto const gyroData = std::get<GyroDataRaw>(fifoMeasurement);
			Serial.print("G: ");
			...
			hasData = true;
		} else if (std::holds_alternative<std::uint64_t>(fifoMeasurement)) {
			auto const timestampData = std::get<std::uint64_t>(fifoMeasurement);
			Serial.print("T: ");
			Serial.println(timestampData);
			hasData = true;
		}

		auto const sensorFusionMeasurement = imu.get_measurement_sensor_fusion();
		if (std::holds_alternative<RotationQuaternion>(sensorFusionMeasurement)) {
			auto const rotationQuaternion = std::get<RotationQuaternion>(sensorFusionMeasurement);
			Serial.print("Q: ");
			Serial.print(rotationQuaternion.rx, 6);
			...
			Serial.println(rotationQuaternion.rw, 6);
			hasData = true;
		} else if (std::holds_alternative<GyroBiasVector>(sensorFusionMeasurement)) {
			auto const gyroBiasVector = std::get<GyroBiasVector>(sensorFusionMeasurement);
			Serial.print("B: ");
			Serial.print(gyroBiasVector.gbx, 6);
			Serial.print(',');
			Serial.print(gyroBiasVector.gby, 6);
			Serial.print(',');
			Serial.println(gyroBiasVector.gbz, 6);
			hasData = true;
		} else if (std::holds_alternative<GravityVector>(sensorFusionMeasurement)) {
			auto const gravityVector = std::get<GravityVector>(sensorFusionMeasurement);
			Serial.print("V: ");
			Serial.print(gravityVector.gvx, 6);
			Serial.print(',');
			Serial.print(gravityVector.gvy, 6);
			Serial.print(',');
			Serial.println(gravityVector.gvz, 6);
			hasData = true;
		}
...
void loop() {
	...
	{  // get imu data
		...
		poll_imu_fifo_all_supported_data();
	}
