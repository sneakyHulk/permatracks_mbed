...
constexpr bool ENABLE_MAG_SENSORS = false;
constexpr bool SYNC_TIME = false;


constexpr std::size_t MAG_SENSOR_COUNT = 111;
constexpr std::int16_t MAG_DUMMY_VALUE = 0;
...
void setup() {
	...
	{
		MagneticFluxDensityDataRawAK09940A mag_sample_size_probe{};
		common2::message("diag fw sizeof(MagneticFluxDensityDataRawAK09940A)=", sizeof(MagneticFluxDensityDataRawAK09940A));
		common2::message("diag fw bytes.size()=", mag_sample_size_probe.bytes.size());
	}
	if constexpr (!ENABLE_MAG_SENSORS) {
		Serial.println("Mag sensors disabled; sending dummy magnetic payload (INT16_MIN).");
	}
	...
	if (SYNC_TIME){  // sync time
		std::tie(time_delay, time_offset) = common2::sync_time();
	}
}

MagneticFluxDensityDataRawAK09940A make_dummy_mag() {
    MagneticFluxDensityDataRawAK09940A d{};
    d.x = MAG_DUMMY_VALUE;
    d.y = MAG_DUMMY_VALUE;
    d.z = MAG_DUMMY_VALUE;
    return d;
...
void loop() {
	...
	{  // poll AK09940A
		...
		if constexpr (ENABLE_MAG_SENSORS) {
			...
			for (std::size_t i = 0; i < MAG_SENSOR_COUNT; ++i) {
				auto const mag = make_dummy_mag();
				Serial.write(mag.bytes.data(), mag.bytes.size());
				crc16.add(mag.bytes.data(), mag.bytes.size());
			}
		}
