...
namespace common {
	...
	template <typename time_type>
	std::tuple<time_type, time_type, time_type> receive_time(std::uint64_t timeout = 100000000ULL) {  // 100ms
	...
	std::tuple<std::uint64_t, std::uint64_t> sync_time() {
	start:
		std::uint64_t average_delay = 0;
		...
		for (auto i = 0; i < 5; ++i) {
			...
			average_delay += ((t3 - t0) - (t2 - t1)) / 2;
			average_offset += ((t1 - t0) + (t2 - t3)) / 2;
		}

		return {average_delay / 5, average_offset / 5};
	}
}  // namespace common