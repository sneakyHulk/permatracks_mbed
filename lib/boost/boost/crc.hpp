...
namespace boost {
	namespace crc_detail {
		...
		template <int Bits>
		struct uint_t : std::conditional<(Bits <= 8), uint_t_8,
		                    typename std::conditional<(Bits <= 16), uint_t_16, typename std::conditional<(Bits <= 32), uint_t_32, typename std::conditional<(Bits <= 64), uint_t_64, uint_t_none>::type>::type>::type>::type {};
...
namespace boost {
	...
	template <std::size_t Bits, BOOST_CRC_PARM_TYPE TruncPoly = 0u, BOOST_CRC_PARM_TYPE InitRem = 0u, BOOST_CRC_PARM_TYPE FinalXor = 0u, bool ReflectIn = false, bool ReflectRem = false>
	class crc_optimal;
	...
	typename crc_detail::uint_t<Bits>::fast augmented_crc(void const *buffer, std::size_t byte_count, typename crc_detail::uint_t<Bits>::fast initial_remainder = 0u);
	...
	class crc_basic {
	   public:
		...
		explicit crc_basic(value_type truncated_polynomial, value_type initial_remainder = 0, value_type final_xor_value = 0, bool reflect_input = false, bool reflect_remainder = false);
	...
	namespace detail {
		...
		struct high_bit_mask_c : std::integral_constant<typename boost::crc_detail::uint_t<BitIndex + 1>::fast, (UINTMAX_C(1) << BitIndex)> {};
		...
		struct low_bits_mask_c : std::integral_constant<typename boost::crc_detail::uint_t<BitCount>::fast, (BitCount ? ((((UINTMAX_C(1) << (BitCount - 1)) - 1u) << 1) | UINTMAX_C(1)) : 0u)> {};
		...
		std::array<unsigned char, (UINTMAX_C(1) << CHAR_BIT)> inline make_byte_reflection_table() {
			...
			do result[i] = reflect_unsigned(i);
			while (++i);
			return result;
		}
		...
		/** \brief  Reflects some bits within a single byte
		    ...
		    \return  The (partially) reflected value.
		 */
		inline unsigned char reflect_sub_byte(unsigned char x, int word_length) { return reflect_byte(x) >> (CHAR_BIT - word_length); }
		 ...
		 */
		template <typename Unsigned>
		inline Unsigned reflect_optionally(Unsigned x, bool reflect, int word_length = std::numeric_limits<Unsigned>::digits) {
		...
		/** \brief  Possibly reflects the bits of a single byte
		    ...
		    \return  <code><var>reflect</var> ? reflect_byte(<var>x</var>) :
		      <var>x</var></code>
		 */
		inline unsigned char reflect_byte_optionally(unsigned char x, bool reflect) { return reflect ? reflect_byte(x) : x; }
		...
		void crc_modulo_word_update(int register_length, Register &remainder, Word new_dividend_bits, Register truncated_divisor, int word_length, bool reflect) {
		...
		inline void crc_modulo_update(int register_length, Register &remainder, bool new_dividend_bit, Register truncated_divisor) {
			crc_modulo_word_update(register_length, remainder, static_cast<unsigned>(new_dividend_bit), truncated_divisor, 1, false);
		}
		 ...
		 */
		template <typename Register, typename Word>
		void augmented_crc_modulo_word_update(int register_length, Register &remainder, Word new_dividend_bits, Register truncated_divisor, int word_length, bool reflect) {
			// Create this masking constant outside the loop.
			Register const high_bit_mask = UINTMAX_C(1) << (register_length - 1);
		 ...
		 */
		template <typename Register>
		inline void augmented_crc_modulo_update(int register_length, Register &remainder, bool new_dividend_bit, Register truncated_divisor) {
			augmented_crc_modulo_word_update(register_length, remainder, static_cast<unsigned>(new_dividend_bit), truncated_divisor, 1, false);
		}
		 ...
		 */
		template <int BitLength>
		class reflector : public std::conditional<(BitLength > CHAR_BIT), super_byte_reflector<BitLength>, sub_type_reflector<BitLength> >::type {};
		...
		std::array<Register, (UINTMAX_C(1) << SubOrder)> make_partial_xor_products_table(int register_length, Register truncated_divisor, bool reflect) {
			std::array<Register, (UINTMAX_C(1) << SubOrder)> result = {0};

			// Loop over every possible dividend value
			for (typename boost::crc_detail::uint_t<SubOrder + 1>::fast dividend = 0u; dividend < result.size(); ++dividend) {
				Register remainder = 0u;

				crc_modulo_word_update(register_length, remainder, dividend, truncated_divisor, SubOrder, false);
				result[reflect_optionally(dividend, reflect, SubOrder)] = reflect_optionally(remainder, reflect, register_length);
			}
			return result;
		}
		...
		class crc_table_t {
		   public:
			...
			static array_type const &get_table() {
				static array_type const table = make_partial_xor_products_table<unit_width_c::value>(width_c::value, poly_c::value, refin_c::value);
		...
		class direct_byte_table_driven_crcs : public crc_table_t<Order, CHAR_BIT, TruncatedPolynomial, false> {
		   ...
		   public:
			 ...
			 */
			static value_type augmented_crc_update(value_type remainder, unsigned char const *new_dividend_bytes, std::size_t new_dividend_byte_count) {
			...
			/** \brief  Compute the updated remainder after reading some bytes
			    ...
			    \return  The updated remainder
			 */
			static value_type crc_update(value_type remainder, unsigned char const *new_dividend_bytes, std::size_t new_dividend_byte_count) {
				static array_type const &table = base_type::get_table();
		...
		class byte_table_driven_crcs : public std::conditional<Reflect, reflected_byte_table_driven_crcs<Order, TruncatedPolynomial>, direct_byte_table_driven_crcs<Order, TruncatedPolynomial> >::type {};
		   ...
		   public:
			...
			static value_type augmented_crc_update(value_type remainder, unsigned char const *new_dividend_bytes, std::size_t new_dividend_byte_count) {
				...
				while (new_dividend_byte_count--) {
					...
					augmented_crc_modulo_word_update(width_c::value, remainder, *new_dividend_bytes++, poly_c::value, CHAR_BIT, false);
				}
			...
			static value_type crc_update(value_type remainder, unsigned char const *new_dividend_bytes, std::size_t new_dividend_byte_count) {
				...
				while (new_dividend_byte_count--) {
					// Without a table, process each byte explicitly
					crc_modulo_word_update(width_c::value, remainder, *new_dividend_bytes++, poly_c::value, CHAR_BIT, false);
				}
		...
		class reflected_sub_byte_crcs : public crc_table_t<Order, Order, TruncatedPolynomial, true> {
		   ...
		   public:
			...
			static value_type augmented_crc_update(value_type remainder, unsigned char const *new_dividend_bytes, std::size_t new_dividend_byte_count) {
				...
				while (new_dividend_byte_count--) {
					// Without a table, process each byte explicitly
					augmented_crc_modulo_word_update(width_c::value, remainder, *new_dividend_bytes++, poly_c::value, CHAR_BIT, true);
				}
				remainder = reflect_sub_byte(remainder, width_c::value);
			...
			static value_type crc_update(value_type remainder, unsigned char const *new_dividend_bytes, std::size_t new_dividend_byte_count) {
				...
				while (new_dividend_byte_count--) {
					// Without a table, process each byte explicitly
					crc_modulo_word_update(width_c::value, remainder, *new_dividend_bytes++, poly_c::value, CHAR_BIT, true);
				}
				remainder = reflect_sub_byte(remainder, width_c::value);
		 ...
		 */
		template <int Order, std::uintmax_t TruncatedPolynomial, bool Reflect>
		class sub_byte_crcs : public std::conditional<Reflect, reflected_sub_byte_crcs<Order, TruncatedPolynomial>, direct_sub_byte_crcs<Order, TruncatedPolynomial> >::type {};
		...
		class crc_driver : public std::conditional<(Order < CHAR_BIT), sub_byte_crcs<Order, TruncatedPolynomial, Reflect>, byte_table_driven_crcs<Order, TruncatedPolynomial, Reflect> >::type {
		   public:
			/** \brief  The type to check for ID
	 ...
	 */
	template <std::size_t Bits>
	inline crc_basic<Bits>::crc_basic(value_type truncated_polynomial,
	    value_type initial_remainder,  // = 0
	    value_type final_xor_value,    // = 0
	    bool reflect_input,            // = false
	    bool reflect_remainder         // = false
	    )
	    : rem_(initial_remainder), poly_(truncated_polynomial), init_(initial_remainder), final_(final_xor_value), rft_in_(reflect_input), rft_out_(reflect_remainder) {}
	...
	inline typename crc_basic<Bits>::value_type crc_basic<Bits>::checksum() const {
		return ((rft_out_ ? detail::reflect_unsigned(rem_, bit_count) : rem_) ^ final_) & detail::low_bits_mask_c<bit_count>::value;
	}
...
#define BOOST_CRC_OPTIMAL_NAME crc_optimal<Bits, TruncPoly, InitRem, FinalXor, ReflectIn, ReflectRem>
	 ...
	 */
	template <std::size_t Bits, BOOST_CRC_PARM_TYPE TruncPoly, BOOST_CRC_PARM_TYPE InitRem, BOOST_CRC_PARM_TYPE FinalXor, bool ReflectIn, bool ReflectRem>
	inline BOOST_CRC_OPTIMAL_NAME::crc_optimal(value_type init_rem  // = initial_remainder
	    )
	 ...
	 */
	template <std::size_t Bits, BOOST_CRC_PARM_TYPE TruncPoly, BOOST_CRC_PARM_TYPE InitRem, BOOST_CRC_PARM_TYPE FinalXor, bool ReflectIn, bool ReflectRem>
	inline void BOOST_CRC_OPTIMAL_NAME::reset(value_type new_rem  // = initial_remainder
	) {
	...
	inline void BOOST_CRC_OPTIMAL_NAME::process_block(void const *bytes_begin, void const *bytes_end) {
		process_bytes(bytes_begin, static_cast<unsigned char const *>(bytes_end) - static_cast<unsigned char const *>(bytes_begin));
	}
	 ...
	 */
	template <std::size_t Bits, BOOST_CRC_PARM_TYPE TruncPoly>
	typename crc_detail::uint_t<Bits>::fast augmented_crc(void const *buffer, std::size_t byte_count,
	    typename crc_detail::uint_t<Bits>::fast initial_remainder  // = 0u
	) {
		return detail::low_bits_mask_c<Bits>::value & detail::byte_table_driven_crcs<Bits, TruncPoly, false>::augmented_crc_update(initial_remainder, static_cast<unsigned char const *>(buffer), byte_count);
	}
