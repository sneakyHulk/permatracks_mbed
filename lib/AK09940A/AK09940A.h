...
class AK09940A final {
   ...
   public:
	...
	void begin() {
		...
		// check sensor
		initialized = check_company_device_id();
		if (!initialized) return;
		initialized = check_company_device_id();
		if (!initialized) return;
		initialized = check_company_device_id();
	...
	[[nodiscard]] MagneticFluxDensityDataRawAK09940A get_measurement() const {
		if (!initialized) {
